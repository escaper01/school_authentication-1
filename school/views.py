        ...
        if form.is_valid():
            ...
            if user is not None:
                if user.is_active:
                    cam = VideoCamera()
                    print('cam init')
                    result = cam.face_checking(username=username, id=user.id)
                    if result:
                        login(request, user)
                        return HttpResponseRedirect('afterlogin')
                    else:
                        custom_error = 'unrecognized'
            else:
                ## user does not exist
                custom_error = 'check your credentials'
        
    return render(request,'school/adminlogin.html', {'form':form,'custom_error': custom_error})
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            ...
            if image_created:
                ...
                messages.success(request, 'Admin Account was successfully registred')
                ...
                u.delete()
    return render(request,'school/adminsignup.html',{'form':form,'custom_error': custom_error})

def custom_student_login(request):
    form=forms.LoginForm()
    logout(request)
    username = password = ''
    custom_error = user = ''
    ...
    return render(request,'school/studentlogin.html', {'form':form,'custom_error': custom_error})
    ...
    if request.method=='POST':
        ...
        if form1.is_valid() and form2.is_valid():
            ...
            if image_created:
                ...
                u = User.objects.get(username = user.username)
                u.delete()
    mydict={'form1':form1,'form2':form2,'custom_error': custom_error}
...
def custom_teacher_login(request):
    form=forms.LoginForm()
    logout(request)
    username = password = ''
    custom_error = user = ''
    ...
    return render(request,'school/teacherlogin.html', {'form':form,'custom_error': custom_error})
